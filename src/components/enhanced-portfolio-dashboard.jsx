...
import { TrendingUp, TrendingDown, AlertCircle, Plus, X, Calendar, ChevronRight, LineChart, DollarSign, Target, Eye } from 'lucide-react';
...
const EnhancedPortfolioDashboard = () => {
  ...
  const stockDatabase = {
    ...
    'SMH': {
      ...
      sector: 'ETF - Semiconductors',
      pe: 18.5,
      eps: null,
      dividendYield: 0.6,
      isETF: true,
      holdings: ['NVDA', 'ASML', 'TSMC', 'QCOM', 'AMD', 'MU', 'LRCX'],
      holdingWeights: [24, 17, 15, 10, 9, 7, 6],
    },
    'TPL': {
    ...
    'BWXT': {
      ...
      pe_vs_sector: 12,
      revenue_growth: 8.9,
      historicalPrices: historicalPrices['BWXT'],
    ...
    'CRWD': {
      ...
      pe_vs_sector: 125,
      revenue_growth: 32.1,
      historicalPrices: historicalPrices['CRWD'],
  ...
  const earningsCalendar = [
    ...
    { symbol: 'SMH', company: 'Semiconductor ETF', date: 'Ongoing', eps_estimate: null, revenue_estimate: null, importance: 'ongoing' },
  ];

  // Portfolio recommendations based on analysis
  const generateRecommendations = () => {
      ...
      return [
        {
          ...
          description: 'Add your first stock to get personalized recommendations based on valuation, growth, and portfolio composition',
          priority: 'high'
        }
      ];
    }

    // Check for high-growth stocks
    const highGrowthStocks = portfolio.filter(s => {
      const stock = stockDatabase[s.symbol];
      return stock && stock.revenue_growth && stock.revenue_growth > 20;
    });

    if (highGrowthStocks.length > 2) {
      recommendations.push({
        type: 'action',
        title: 'Reduce Growth Concentration',
        description: `You have ${highGrowthStocks.length} high-growth stocks (>20% revenue growth). Consider adding value or dividend stocks for balance.`,
        affectedStocks: highGrowthStocks.map(s => s.symbol),
        priority: 'medium'
      });
    }

    // Check for value opportunities
    const valueStocks = portfolio.filter(s => {
      const stock = stockDatabase[s.symbol];
      return stock && stock.pe && stock.pe < 15 && stock.dividendYield > 2;
    });

    if (valueStocks.length === 0 && portfolio.length > 0) {
      recommendations.push({
        type: 'opportunity',
        title: 'Add Dividend/Value Stocks',
        description: 'Consider adding dividend-paying value stocks (P/E < 15, Div > 2%) to provide portfolio stability and income.',
        actionItems: ['SCCO', 'FTAI', 'VRT'],
        priority: 'medium'
      });
    }
    ...
    const heavySector = Object.entries(sectors).find(([_, count]) => count > 3);
    if (heavySector) {
      recommendations.push({
        type: 'warning',
        title: `Sector Concentration: ${heavySector[0]}`,
        description: `You have ${heavySector[1]} stocks in ${heavySector[0]}. Consider diversifying to reduce sector risk.`,
        priority: 'high'
      });
    }
  ...
  const getPortfolioStats = () => {
    ...
    portfolio.forEach(stock => {
      ...
      const data = stockDatabase[stock.symbol];
      if (data) {
        const sector = data.sector;
        sectors[sector] = {
          ...sectors[sector],
          value: (sectors[sector]?.value || 0) + value,
          count: (sectors[sector]?.count || 0) + 1
        };
      }
    });
    ...
    if (newStock.symbol && newStock.quantity) {
      ...
      if (stock) {
        setPortfolio([...portfolio, {
          ...
          price: 100, // Would be fetched from API
          ...stock
        }]);
      ...
      {showApiSetup && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-700 rounded-xl max-w-md w-full p-8">
            <h2 className="text-2xl font-bold text-cyan-400 mb-4">Finnhub API Setup (Optional)</h2>
            <p className="text-slate-300 text-sm mb-6">
              Connect your Finnhub API key for real-time stock prices, news, and analyst ratings. Get a free key at <a href="https://finnhub.io" className="text-cyan-400 hover:underline">finnhub.io</a>
            </p>
            <input
              type="password"
              placeholder="Enter your Finnhub API key (or skip)"
              value={apiKey}
              onChange={(e) => setApiKey(e.target.value)}
              className="w-full bg-slate-800 border border-slate-600 rounded px-3 py-2 text-white placeholder-slate-400 focus:border-cyan-500 focus:outline-none mb-4"
            />
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 bg-cyan-600 hover:bg-cyan-700 px-4 py-2 rounded font-medium transition-colors"
              >
                {apiKey ? 'Connect' : 'Skip'}
              </button>
            </div>
            <p className="text-xs text-slate-500 text-center mt-4">You can update this in settings later</p>
          </div>
        </div>
      )}
        ...
        <div className="max-w-7xl mx-auto px-8 py-6">
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-slate-400 text-sm mt-1">Long-term Growth Analytics & Recommendations</p>
            </div>
            <button
              onClick={() => setShowAddStock(!showAddStock)}
              className="bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700 px-6 py-3 rounded-lg flex items-center gap-2 transition-all"
            >
      ...
      <div className="max-w-7xl mx-auto px-8 py-8">
        ...
        {portfolio.length > 0 && activeTab === 'overview' && (
          <div className="grid grid-cols-4 gap-6 mb-8">
            ...
            <div className="bg-gradient-to-br from-slate-800 to-slate-900 p-6 rounded-xl border border-slate-700">
              ...
              <p className={`text-3xl font-bold ${parseFloat(stats.totalGain) >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                ${stats.totalGain}
              </p>
            </div>
            <div className="bg-gradient-to-br from-slate-800 to-slate-900 p-6 rounded-xl border border-slate-700">
              <p className="text-slate-400 text-sm mb-2">Return %</p>
              <p className={`text-3xl font-bold ${parseFloat(stats.gainPercent) >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                {stats.gainPercent}%
              </p>
            </div>
          </div>
            ...
            <div className="grid grid-cols-3 gap-4 mb-4">
              <div>
                ...
                <datalist id="stock-options">
                  {Object.keys(stockDatabase).map(symbol => (
          ...
          portfolio.length > 0 ? (
              ...
              {portfolio.map(stock => {
                ...
                return (
                  <div
                    ...
                    className="bg-gradient-to-br from-slate-800 to-slate-900 border border-slate-700 rounded-xl p-5 hover:border-cyan-500 transition-all cursor-pointer hover:shadow-lg hover:shadow-cyan-500/20"
                  >
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs mb-4">
                      <div>
                        ...
                        <p className="font-bold text-cyan-400">{stock.pe ? stock.pe.toFixed(1) : 'N/A'}</p>
                      </div>
                      <div>
                        <p className="text-slate-400">Div Yield</p>
                        <p className="font-bold text-cyan-400">{stock.dividendYield ? stock.dividendYield.toFixed(1) : '0'}%</p>
                      </div>
                    </div>
            ...
            <div className="text-center py-16">
              ...
              <button
                ...
                className="bg-cyan-600 hover:bg-cyan-700 px-6 py-2 rounded-lg transition-colors mt-4"
              >
        ...
        {activeTab === 'events' && (
          <div className="bg-slate-800 border border-slate-700 rounded-xl overflow-hidden">
            <div className="bg-slate-900 border-b border-slate-700 px-6 py-4 flex items-center gap-2">
              ...
              <h2 className="text-xl font-bold">Earnings Calendar & Events</h2>
            </div>
                ...
                <div key={i} className="p-6 hover:bg-slate-900/50 transition-colors">
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <div className="text-right">
                      ...
                      {event.eps_estimate && (
                        <p className="text-xs text-slate-400">EPS: ${event.eps_estimate}</p>
                      )}
                    </div>
                  </div>
                  {event.revenue_estimate && (
                    <p className="text-sm text-slate-400">Revenue Estimate: {event.revenue_estimate}</p>
                  )}
                  {portfolio.find(s => s.symbol === event.symbol) && (
            ...
            {recommendations.map((rec, i) => (
              <div
                ...
                className={`border rounded-xl p-6 ${
                  ...
                  rec.type === 'action' ? 'bg-yellow-500/10 border-yellow-500/30' :
              ...
              >
                <div className="flex items-start gap-3 mb-3">
                  <AlertCircle size={20} className={
                    rec.type === 'warning' ? 'text-red-400 flex-shrink-0 mt-1' :
                    rec.type === 'action' ? 'text-yellow-400 flex-shrink-0 mt-1' :
                    rec.type === 'opportunity' ? 'text-green-400 flex-shrink-0 mt-1' :
                  ...
                  <div>
                    ...
                    {rec.actionItems && (
                      <div className="mt-3 flex gap-2">
                        {rec.actionItems.map(symbol => (
                          <span key={symbol} className="px-3 py-1 bg-slate-700 rounded text-xs text-slate-200">
                            {symbol}
                          </span>
                        ))}
                      </div>
        ...
        {activeTab === 'analysis' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {Object.keys(stats.sectors).length > 0 && (
              <div className="bg-slate-800 border border-slate-700 rounded-xl p-6">
                <h3 className="text-lg font-bold text-cyan-400 mb-4 flex items-center gap-2">
                  <LineChart size={20} /> Sector Allocation
                </h3>
                <div className="space-y-3">
                  {Object.entries(stats.sectors).map(([sector, data]) => {
                    const pct = (data.value / parseFloat(stats.totalValue) * 100).toFixed(1);
                    return (
                      <div key={sector}>
            ...
            <div className="bg-slate-800 border border-slate-700 rounded-xl p-6">
              <h3 className="text-lg font-bold text-cyan-400 mb-4 flex items-center gap-2">
                <DollarSign size={20} /> Portfolio Metrics
              </h3>
              <div className="space-y-4">
                <div>
                    ...
                    {selectedStock.historicalPrices.map((ph, i) => {
                      ...
                      return (
                        <div key={i} className="flex-1 flex flex-col items-center gap-2 min-w-12">
                          <div
                            className="w-full bg-gradient-to-t from-cyan-500 to-blue-500 rounded-t hover:opacity-80 transition-opacity cursor-pointer relative group"
                            style={{ height: `${Math.max(height, 5)}%` }}
                          >
                            <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 hidden group-hover:block bg-slate-950 text-cyan-400 text-xs px-2 py-1 rounded whitespace-nowrap border border-slate-600">
                              ${ph.price.toFixed(2)}
                            </div>
                          </div>
                          <span className="text-xs text-slate-500 text-center line-clamp-1">{ph.date.split('/')[0]}</span>
                        </div>
                      );
                    })}
                  </div>
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="bg-slate-800 border border-slate-700 rounded-lg p-6">
                  ...
                  <div className="space-y-3 text-sm">
                    ...
                    {selectedStock.revenue_growth && (
                      <div className="flex justify-between pt-3 border-t border-slate-700">
                        <span className="text-slate-400">Revenue Growth</span>
                        <span className="font-semibold text-green-400">{selectedStock.revenue_growth.toFixed(1)}%</span>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="bg-slate-800 border border-slate-700 rounded-lg p-6">
                  <h3 className="text-lg font-bold text-purple-400 mb-4">Valuation</h3>
                  <div className="space-y-3 text-sm">
                    <div className="flex justify-between">
                      <span className="text-slate-400">P/E vs Sector</span>
                      <span className={`font-semibold ${selectedStock.pe_vs_sector ? (selectedStock.pe_vs_sector < 0 ? 'text-green-400' : 'text-red-400') : 'text-slate-400'}`}>
                        {selectedStock.pe_vs_sector ? `${selectedStock.pe_vs_sector > 0 ? '+' : ''}${selectedStock.pe_vs_sector}%` : 'N/A'}
                      </span>
                    </div>
                    {selectedStock.operatingMargin && (
                      <div className="flex justify-between">
                        <span className="text-slate-400">Operating Margin</span>
                        <span className="font-semibold text-white">{selectedStock.operatingMargin.toFixed(1)}%</span>
                      </div>
                    )}
                    {selectedStock.netMargin && (
                      <div className="flex justify-between">
                        <span className="text-slate-400">Net Margin</span>
                        <span className="font-semibold text-white">{selectedStock.netMargin.toFixed(1)}%</span>
                      </div>
                    )}
                  </div>
                </div>
                    ...
                    {selectedStock.holdings.map((holding, i) => (
                      <div key={holding} className="flex justify-between items-center">
                        ...
                        <div className="flex-1 mx-3 bg-slate-600 rounded-full h-2">
                          <div
                            className="bg-gradient-to-r from-cyan-500 to-blue-500 h-2 rounded-full"
                            style={{ width: `${selectedStock.holdingWeights[i]}%` }}
                          />
                        </div>
